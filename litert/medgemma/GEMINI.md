...
- **Tools Priority**: Use `mediapipe` for the final bundle and `ai-edge-torch` if a custom SigLIP conversion is needed. Use context7 to utilize recent libraries.
- **Constraints**:
...
- **Environment**: Apple Silicon M4 (Unified Memory)
- **Virtual Environment**: Create a new uv python virtual environment inside the medgemma directory namely `.mgenv` (e.g., `uv venv .mgenv`).
- **Target**: MediaPipe Task Bundle (.bin) with 4-bit quantization.
- **Rules**:
