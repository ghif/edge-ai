...
* **Split-Brain Tracing (CPU Focus):** Ensure all models, tensors, and dummy inputs are explicitly moved to `cpu` (e.g., `torch.set_default_device('cpu')`) before calling `ai_edge_torch.convert()`.
* **Dependencies:** Ensure PyTorch 2.6.x+ and a standard `jaxlib` are installed. CUDA is not required and should be avoided to minimize system overhead.
...
- **Rules**:
  ...
  3. **Critical**: MedGemma 1.5 has a 3D SigLIP encoder. All tracing must be performed on CPU.
  ...
  6. **Compatibility**: Ensure all installed Python libraries are compatible with CPU-only execution.
  7. **Execution Mode**: Execute the task with `--yolo` mode enabled for accelerated processing.
  8. **Conversion Device**: Always use `cpu` mode for conversion attempts. Explicitly disable CUDA if detected.
